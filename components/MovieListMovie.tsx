  ...
  return (
    <li className="py-4 flex items-center justify-between">
      ...
      <Link href={`/movies/${movie.id}`}>
       ...
       <div>
        <span className={`rounded-full text-white flex items-center justify-center h-8 w-8 md:h-12 md:w-12 md:text-xl font-bold md:font-extrabold ${movie.vote_average < 5 ? 'bg-green-900' : movie.vote_average < 8 ? 'bg-green-700' : 'bg-green-500'}`}>
          {movie.vote_average}
          </span>
       </div>
    </li>
